...
function build() {
  ...
  const bodyHtml = `
    ...
    ${step(5, 'พิมพ์คำถามทดสอบใน Teams', `
      ...
      ${mockup(mock6, 'ตัวอย่างแบบเดียวกัน แต่คนละแผนก: Sales Data Bot ของทีม Sales Engineer ใน Teams — ตอบยอดขายเจาะจงจุดเดียวได้ถูกต้อง (ลองคำถามรวมยอดข้ามเดือน/สาขาดูด้วย จะเห็นข้อจำกัดของ RAG ตามที่คุยกันใน Lab 5)')}
  ...
  const html = pageShell({
    file: 'lab7.html',
    title: 'Lab 7: Deploy to Teams — RAG Course',
    eyebrow: 'LAB 7 · ⏱ 15 นาที · Level 100',
    ...
    prevNext: { prev: { href: 'lab6.html', label: 'Lab 6: Actions & Tools' }, next: { href: 'index.html', label: 'กลับหน้าแรก 🎉' } },
    bodyHtml,
  });
  writePage('lab7.html', html);
}
